                ...
                <div class="head">
                  ...
                  <div>
                    ...
                    <el-button
                      ...
                      circle
                      @click="updownContent(n, 1)"
                    >
                    </el-button>
                  </div>
                </div>
                <el-form v-if="v.type == 'essay'" ref="form">
                  ...
                  <el-form-item label="最小字数">
                    <el-input
                      style="width: 200px"
                      v-model.number="content[n].minWord"
                    ></el-input>
                  </el-form-item>
                  <el-form-item label="最大字数">
                    <el-input
                      style="width: 200px"
                      v-model.number="content[n].maxWord"
                    ></el-input>
                  </el-form-item>
            ...
            <el-card class="configcss" shadow="hover">
              <el-form ref="form" label-width="80px">
                ...
                <el-form-item
                  inline-message
                  label="补充信息"
                  v-if="config.info.show == false"
                >
                  姓名:
                  <el-switch v-model="config.info.name"></el-switch>&nbsp;&nbsp;
                  学号:
                  <el-switch v-model="config.info.number"></el-switch
                  >&nbsp;&nbsp; 班级:
                  <el-switch v-model="config.info.class"></el-switch
                  >&nbsp;&nbsp; 专业:
                  <el-switch v-model="config.info.profession"></el-switch> 学校:
                  <el-switch v-model="config.info.school"></el-switch>
                </el-form-item>
                <el-form-item label="答题内容">
                  <el-input
                    style="width: 200px"
                    v-model.number="config.autoSave"
                  ></el-input>
                  秒自动保存
                </el-form-item>
...
export default {
  ...
  created() {
    ...
    this.config.endTime = Date.now() + 86400000;
  },
  methods: {
    addContent(val) {
      // 添加题型
    ...
    saveForm() {
      ...
      //   console.log(this.config.endTime);
      //   return;
      this.config.startTime = new Date(this.config.startTime).getTime();
    ...
    updownContent(n, type) {
      ...
      if (type == 0) {
        ...
        [this.content[n], this.content[n - 1]] = [
          this.content[n - 1],
          this.content[n],
        ];
      } else {
        ...
        [this.content[n + 1], this.content[n]] = [
          this.content[n],
          this.content[n + 1],
        ];
      }
    ...
    endTimeChange() {
      // 判断截止时间
      console.log("aaaa");
      const a = new Date(this.config.startTime).getTime();
      const b = new Date(this.config.endTime).getTime();
      const now = Date.now();
      if (b <= a) {
        this.$message.error("截止时间需要大于开始时间");
        return;
      }
      if (b <= now) {
        this.$message.error("截止时间需要大于当前时间");
        return;
      }
    },
    delContent(n) {
    ...
    gohref() {
      window.open(
        `http://py.jukuu.com:8080/zhihui/#/taswrite?id=${this.id}`,
        "_blank"
      );
    },
  },
