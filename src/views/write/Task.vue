                ...
                <div class="contentCard">
                    <div style="flex:1;">
                        <div v-for="v,n in content" :key="n">
                            <template v-if="publish.config.onePage ==true?n == contentNum : true">
                                <div style="display:flex;justify-content:space-between;align-items:center;">
                                    <p class="title">{{n+1}}. 写作</p>
                                    <div style="margin-right:20px;">
                                        <template v-if="publish.config.onePage ==true">
                                            <el-button type="primary" plain size="mini" icon="el-icon-arrow-left" circle @click="contentNum>0?contentNum--:false" >
                                            </el-button>
                                            <el-button type="primary" plain size="mini" icon="el-icon-arrow-right" circle @click="contentNum<(content.length-1)?contentNum++:false" >
                                            </el-button>
                                        </template>
                                    </div>
                                </div>
                                <div>
                                    <el-card class="card request" shadow="hover" body-style="padding:15px 20px;">
                                        <p>要求：</p>
                                        <div>
                                            <p style="text-align:center">{{publish.content[n].title}}</p>
                                            <p style="text-align:left;text-indent: 10px;">{{publish.content[n].request}}</p>
                                        </div>
                                    </el-card>
                                    <el-card class="card"  shadow="hover" body-style="padding:15px;">
                                        <div class="titlesW">
                                        <el-input v-model="content[n].essay_title" placeholder="请输入标题"></el-input>
                                        </div>
                                        <div id="contentsW">
                                                <div id="contentsW_bar">
                                                    <div class="contentsW_bar_left">请在下方写入 正文</div>
                                                </div>
                                                <textarea style="margin-bottom: -5px;" placeholder="请输入正文......." class="textarea content_line" cols="70" rows="10" v-model="content[n].essay_or_snts"></textarea>
                                                <!-- 草稿自动保存底部栏 -->
                                                <div class="from_bar">
                                                    <div>
                                                        <span>字数:<span id="init_wc">{{spss_countwords(content[n].essay_or_snts)}} </span> 词</span> &nbsp;
                                                    </div>
                                                    <div class="fr">
                    ...
                    <div class="answerCard" v-if="publish.config.showCard">
                        ...
                        <div style="display:flex;flex-wrap:wrap;">
                            <div class="answerItem" :class="isshowAnswerColor(n)" @click="contentNum=n" v-for="v,n in content.length" :key="v">
                                {{v}}
                            </div>
                        </div>
                    </div>
         ...
         goSubmit(state){
            if(state){
                ...
                for(let key in this.content){
                    if(this.spss_countwords(this.content[key].essay_or_snts)<this.publish.content[key].minWord){
                        this.$message.error(`第${Number(key)+1}题正文内容请大于${this.publish.content[key].minWord}个词`)
                        return;
                    }
                     if(this.spss_countwords(this.content[key].essay_or_snts)>this.publish.content[key].maxWord){
                        this.$message.error(`第${Number(key)+1}题正文内容请小于${this.publish.content[key].maxWord}个词`)
                        return;
                    }
                }
